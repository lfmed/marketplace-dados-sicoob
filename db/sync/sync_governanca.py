...
Lê as tabelas Delta do Motor (governanca + gestao_acesso no UC) e faz UPSERT por PK nos
schemas mirror do Lakebase. Stand-in do synced-table nativo (cliente usa native_sync_setup).
...
from db.seed.gov_schema import GOV_TABLES, coerce_from_delta  # noqa: E402


def upsert(cur, schema, tabela, cols, pk, valores):
    ph = ",".join(["%s"] * len(cols))
    non_pk = [c for c in cols if c not in pk]
    if non_pk:
        sets = ",".join(f"{c}=EXCLUDED.{c}" for c in non_pk)
        sql = (f'INSERT INTO {schema}.{tabela} ({",".join(cols)}) VALUES ({ph}) '
        ...
        with conn.cursor() as cur:
            for schema, tabela, cols, pk in GOV_TABLES:
                ...
                for r in rows:
                    ...
                    upsert(cur, schema, tabela, cols, pk, valores)
                if rows:
                    total += len(rows)
                    print(f"  {schema}.{tabela}: {len(rows)} linhas sincronizadas")
        conn.commit()
