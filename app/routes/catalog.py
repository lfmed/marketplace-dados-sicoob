...
def catalogo():
    ...
    ativos = catalog_service.listar_ativos(id_dominio, id_subdominio, tipo, busca)
    return render_template(
        "catalogo.html",
        ativos=ativos,
        dominios=catalog_service.listar_dominios(),
        subdominios=catalog_service.listar_subdominios(id_dominio),
        tipos=catalog_service.TIPOS_ATIVO, tipo_label=catalog_service.TIPO_LABEL,
        f_dominio=id_dominio, f_subdominio=id_subdominio, f_tipo=tipo, f_busca=busca,
        total=len(ativos),
    )
