    ...
    return db.query(
        f"""WITH RECURSIVE sub AS (
             SELECT id_usuario_aisn FROM {ACC}.hierarquia_usuario
              WHERE id_gestor_aisn=%s AND bol_atual=true AND bol_excluido=false
             UNION
             SELECT h.id_usuario_aisn FROM {ACC}.hierarquia_usuario h
...
def _e_proprietario_grupo(id_grupo, id_usuario):
    if not id_grupo:
        return False
    return db.query_one(
        f"""SELECT 1 AS ok FROM {ACC}.grupo_acesso_proprietario
             WHERE id_grupo_acesso=%s AND id_usuario_aisn=%s
               AND bol_atual=true AND bol_excluido=false LIMIT 1""",
        (id_grupo, id_usuario)) is not None


def decidir_autorizacao(id_solicitacao, id_gestor, aprovar, justificativa=None):
    s = db.query_one(f"SELECT * FROM {APP}.solicitacao_acesso WHERE id_solicitacao_acesso=%s",
                     (id_solicitacao,))
    if not s:
    ...
    with db.get_conn(autocommit=False) as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"""INSERT INTO {APP}.autorizacao_hierarquica
...
def decidir_aprovacao_owner(id_solicitacao, id_owner, aprovar, justificativa=None):
    ...
    with db.get_conn(autocommit=False) as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"""INSERT INTO {APP}.aprovacao_owner
